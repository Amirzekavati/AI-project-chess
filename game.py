...
class Board:
    ...
    def __apply_move__(self, queen_move):
        ...
        if push:
            ...
            self.__board_state__[int(opponent_new_pos[0])][int(opponent_new_pos[1])] = \
        ...
        self.__board_state__[int(row)][int(col)] = self.__queen_symbols__[self.__active_players_queen__]

        if self.move_is_in_board(my_pos[0], my_pos[1]):
            self.__board_state__[int(my_pos[0])][int(my_pos[1])] = Board.BLOCKED
    ...
    def is_spot_open(self, row, col):
        return self.__board_state__[int(row)][int(col)] == Board.BLANK
    def is_spot_queen(self, row, col):
        ...
        return self.__board_state__[int(row)][int(col)] == q1 or self.__board_state__[int(row)][int(col)] == q2
    ...
    def move_is_legal(self, row, col):
        return 0 <= row < self.height and \
            ...
            self.__board_state__[int(row)][int(col)] == Board.BLANK
    ...
    def __apply_move_write__(self, move_queen):
        ...
        self.__board_state__[int(row)][int(col)] = \
